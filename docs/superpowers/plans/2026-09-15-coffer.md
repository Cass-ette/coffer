...
- Create: `app/Sources/Settings/SettingsView.swift`（最小版，Task 18 完整化）
...
struct SettingsView: View {
    var body: some View {
        Text("设置（Task 18 完整实现）")
            .frame(width: 420, height: 260)
    }
}
```

注：此为 Task 18 将整体替换的最小实现，仅保证 Settings 场景可编译。
...
struct SettingsView: View {
    ...
    private static let lockChoices: [(label: String, seconds: Int)] = [
        ("1 分钟", 60), ("5 分钟", 300), ("15 分钟", 900), ("不自动", 0),
    ]
    private static let clipboardChoices: [(label: String, seconds: Int)] = [
        ("30 秒", 30), ("45 秒", 45), ("60 秒", 60), ("不清空", 0),
    ]
    ...
    var body: some View {
        ...
        .onChange(of: appSettings.hotkey) { app.reRegisterHotkey() }
    ...
    private var securitySection: some View {
        Section("安全") {
            Picker("闲置自动锁定", selection: $autoLockSeconds) {
                ForEach(Self.lockChoices, id: \.seconds) { Text($0.label).tag($0.seconds) }
            }
            Picker("剪贴板清空", selection: $clipboardSeconds) {
                ForEach(Self.clipboardChoices, id: \.seconds) { Text($0.label).tag($0.seconds) }
            }
