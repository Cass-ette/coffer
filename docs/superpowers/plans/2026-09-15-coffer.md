...
9. 无备份损坏路径：`rm ~/Library/Application\ Support/Coffer/vault.bak1` 后重开 app → 恢复页**不出现**"从备份恢复"按钮；点"新建空库…"出现"确认放弃旧数据并新建"的二次确认（验到弹二次确认即止，不必真点）
...
11. Task 13 补验——显形切换：点眼睛图标密码在明文/掩码间切换；敏感自定义字段同样默认打码
