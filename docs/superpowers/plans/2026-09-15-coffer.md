...
struct PasswordGeneratorView: View {
    ...
    @State private var lowers = true, uppers = true, digits = true, symbols = true
...
3. 新建 totp：粘贴 `otpauth://totp/x?secret=JBSWY3DPEHPK3PXP` → 表单即时显示验证码 `482916` 附近跳动的 6 位数 → 保存 → 详情页大码 + 倒计时环
...
private struct KeyCatcher: NSViewRepresentable {
    ...
    static func dismantleNSView(_ nsView: CaptureView, coordinator: Context) {
        nsView.endCapture()
    }
