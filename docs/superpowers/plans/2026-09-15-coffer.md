...
**规范审查（第一轮）：** ❌ 发现 9 个问题：
1. FuzzyMatch 参数名：`score(query:target:)` → 应为 `score(query:in:)`
2. 空查询行为：返回 `nil` → 应返回 `0`
3. 子串评分公式错误：`1000 + prefixBonus(200) - lengthPenalty(t.count)` → 应为 `1000 + max(0, 100-(t.count-q.count)) + prefixBonus(50)`
4. 子序列评分公式错误：`500 + maxStreak*10 - gaps*5` → 应为 `100 + matched*10 + bestStreak*15 - gaps*2`
5. SearchIndex.search() 参数：`tags: [String]` → 应为 `tag: String?`
6. 字段权重错误：apiKey.provider(60→40), sshKey.host(60→40), permissionNote(20→25), customField(30→20)
7. 评分计算缺失：`score * weight` → 应为 `score * weight / 100`
8. 测试参数名错误：`query:target:` → 应为 `query:in:`
9. sshKey.user 字段可搜索 → spec 仅列出 host，应移除

修复提交：d0931e5 "fix(search): align with spec - parameter names, scoring formulas, and weights"

**规范审查（第二轮）：** ✅ 全部 9 个问题已修复，规范合规。

**质量审查（第一轮）：** ❌ 发现 1 Critical 问题：
- **评分归一化错误** (SearchIndex.swift 66-129 行)：所有评分计算错误地除以 100，将所有权重归一化到 ~1.0 范围而非作为乘数应用。`s * weight / 100` 应为 `s * weight`。影响全部字段权重（title 100, tags 80, URLs 60, subtitle 50, provider/host 40, permissionNote 25, customFields 20）。测试通过是因为仅验证相对顺序，未验证绝对分值。

修复提交：91b3999 "fix(search): remove incorrect /100 normalization from score calculations"

**质量审查（第二轮）：** ✅ Critical 问题已修复，所有评分计算正确使用 `s * weight` 无 `/100` 除法。

**最终状态：**
- 三次提交：8dd74ff (初始实现) → d0931e5 (规范修复) → 91b3999 (质量修复)
- 全量测试：46/46 通过（5 FuzzyMatch + 5 SearchIndex + 36 前序任务）
- 架构清晰：FuzzyMatch 纯评分函数，SearchIndex 处理过滤和排序
- Sendable 合规：SearchIndex 正确声明 Sendable 用于并发访问
...
**规范审查（第一轮）：** ❌ 发现 6 个问题：
1. 缺少 import CryptoKit
2. DEK 类型必须是 SymmetricKey，而非 Data
3. contains() 不应 throws（spec 1728 行显示无 throws）
4. DEKStoreError 必须遵循 LocalizedError（spec 1732-1738 行）
5. 测试错误检查应使用数字 -34018，而非 errSecMissingEntitlement
6. FakeDEKStore 缺少 peek() 方法（spec 1697 行）

修复提交：fbec29e "fix(dek-store): align with spec - SymmetricKey type, LocalizedError, peek() method"

**规范审查（第二轮）：** ✅ 全部 6 个问题已修复，规范合规。

**质量审查（第一轮）：** ❌ 发现 1 Important 问题：
- **FakeDEKStore 缺少 willReject 属性** — Task 9 (UnlockService) 需要测试"用户取消生物认证"流程，但 FakeDEKStore 无法模拟认证拒绝。Plan 1688 行显示 `fake.willReject = true` 用法。需添加 `var willReject = false` 属性，`retrieve(using:)` 返回 `willReject ? nil : storage`。

修复：已追加到 fbec29e commit（amend）

**质量审查（第二轮）：** ✅ Important 问题已修复，FakeDEKStore 现已具备 willReject 能力用于 Task 9 测试。

**最终状态：**
- 两次提交：c75280c (初始实现) → fbec29e (规范+质量修复，包含 willReject)
- 全量测试：48/48 通过，1 跳过（SystemDEKStore 在 -34018 时自动跳过，符合预期）
- 安全实现：kSecAttrAccessControl(.userPresence) + kSecAttrAccessibleWhenUnlockedThisDeviceOnly
- 错误处理：retrieve() 对 5 种预期认证失败返回 nil（errSecItemNotFound/errSecAuthFailed/errSecUserCanceled/errSecInteractionNotAllowed/errSecDecode），仅非预期错误抛出
- 幂等删除：delete() 接受 errSecSuccess 或 errSecItemNotFound
- 测试基础设施：FakeDEKStore 配备 peek() 和 willReject，就绪用于 Task 9
...
**规范审查：** ✅ 全部通过，完全符合规范。
- BiometryPrompting 协议：class-bound，async authenticatedContext 方法
- TouchIDPrompt：真实现使用 LAContext.evaluatePolicy(.deviceOwnerAuthentication)
- UnlockResult enum：5 个 case（unlocked, needMasterPassword, wrongPassword, cancelled, failed），Equatable 遵循
- UnlockService 类：ObservableObject，@Published 状态，所有方法正确实现
- 8 个测试全部存在且正确实现
- FakeDEKStore.stored 属性已暴露用于 Keychain 丢失模拟
- 初始提交：3a194c8

**质量审查（第一轮）：** ❌ 发现 2 Critical 问题：
1. **restoreFromLatestBackup 逻辑 bug** (100 行)：变量 `backup` 被捕获但从未使用，实现立即调用 `restoreFromBackup(1)` 而不验证。这是逻辑错误。
2. **finishUnlock reload 行为** (148-155 行)：`finishUnlock()` 中调用 `loadEnvelope()`，plan 中未显示此调用。这改变了行为 — 每次解锁尝试都从磁盘重新加载 envelope，可能掩盖状态不一致或引入竞态条件。

**缺失测试覆盖：**
- testRestoreFromLatestBackup
- testDiscardVaultAndBackups

修复提交：51fb62b "fix(unlock-service): remove finishUnlock reload + fix restoreFromLatestBackup + add 2 tests"

**质量审查（第二轮）：** ✅ 两个 Critical 问题已修复，2 个测试已添加，代码质量通过。

**最终状态：**
- 两次提交：3a194c8 (初始实现) → 51fb62b (质量修复 + 测试补全)
- 全量测试：58/58 通过，1 跳过（48 前序 + 10 UnlockService 测试）
- 解锁流程：生物识别优先（contains 检查 → 认证 → 提取 DEK → finishUnlock），主密码回退
- 锁定清理：清除 isUnlocked, document, dek（best-effort zeroing）
- 状态管理：ObservableObject + @Published 用于 SwiftUI 集成
- 完整测试覆盖：建库、生物解锁（成功/取消）、Keychain 回退、错误密码、锁定、持久化、密码更改、备份恢复、丢弃所有数据
...
- 创建 AppSettings.swift：ObservableObject 单例，管理启动前设置（hotkey, launchAtLogin）
- HotkeyModifier enum：command, option, control, shift（Codable, CaseIterable）
- Hotkey struct：keyCode + modifiers，默认 ⌥Space (keyCode 49)
- LaunchAtLogin enum：封装 SMAppService.mainApp 的 register/unregister，开发环境静默失败（符合预期）
- 编译验证：✅ `** BUILD SUCCEEDED **`
- 提交：cc8ec39 "feat(app): pre-unlock settings layer (hotkey, launch at login)"

无单测（纯 UserDefaults 读写），以 Task 11 起的实际使用验证。
...
- 创建 7 个文件：AppState.swift, CofferApp.swift(替换), FirstRunView.swift, RestoreView.swift, LockScreenView.swift, MainView.swift, SettingsView.swift
- AppState: Phase enum (firstRun/corrupted/locked/unlocked), UnlockService 依赖装配, probe() 初始化
- CofferApp: Window + Settings 场景，RootView 根据 phase 路由到对应视图
- FirstRunView: 主密码创建流程（≥8位，匹配验证）
- RestoreView: 损坏恢复页（备份恢复/放弃重建）
- LockScreenView: 最小锁定页（仅主密码解锁，Task 12 将添加指纹）
- MainView: 最小主界面（已解锁提示 + 锁定按钮）
- SettingsView: 占位符（Task 17 完整实现）
- 关键调整：import CofferCore（UnlockService 属于 CofferCore 模块），正确初始化 VaultStore/SystemDEKStore/TouchIDPrompt
- 编译验证：✅ `** BUILD SUCCEEDED **`
- 提交：11a06ec "feat(app): app state machine, first-run, corruption recovery, minimal lock/main"

手动验证留待 Task 13 完成后统一进行（计划 lines 2582-2592）。
...
**Commits:**
- ab614f9: feat(app): full lock screen with Touch ID + master password fallback
- a70ba41: fix(app): correct unlock state transitions and cache biometrics check

**Spec Compliance Review:** ✅ Passed
- All requirements implemented correctly
- Touch ID as primary method with master password fallback
- Proper state management and UI flow
- Error handling for both unlock modes

**Code Quality Review (First Pass):** ❌ Critical Issues Found
1. State machine broken - treating non-throwing enum-returning functions as throwing, never transitioning to `.unlocked` phase
2. Performance bug - `LAContext()` created on every View render

**Code Quality Review (After Fixes):** ✅ Approved
- State machine corrected with proper `UnlockResult` enum handling
- Performance issue resolved by caching biometrics check in `@State`
- All critical issues fixed, no regressions introduced

---

**Task 13 Implementation Results:**

**Commits:**
- f7b6d1f: feat(app): three-column main UI with clipboard protection and TOTP display
- a4f4a39: perf(app): inline SearchIndex creation to avoid recreation on every render

**Spec Compliance Review:** ✅ Passed
- All 7 files created/modified correctly (ClipboardManager, EntryTypeIcon, MainView, SidebarView, EntryListView, EntryDetailView, TOTPCodeView)
- Clipboard auto-clear with paste verification implemented
- Three-column NavigationSplitView with proper SearchIndex integration
- TOTP live display with countdown
- All payload types handled in detail view with reveal/copy actions

**Code Quality Review (First Pass):** ✅ Approved with recommendation
- Excellent separation of concerns across all components
- Security-conscious clipboard handling
- Good defensive programming and error handling
- One performance issue identified: SearchIndex recreation on every render

**Code Quality Review (After Fix):** ✅ Approved
- Performance issue fixed by inlining SearchIndex creation in visibleEntries
- SearchIndex now only created when dependencies (entries/section/query) change
- Minimal, clean fix with no regressions
...
**Commits:**
- b0350c9: feat(app): entry editor with password generator and CRUD operations
- a4e113f: fix(app): use enumerated pattern for stable ForEach indices

**Spec Compliance Review:** ✅ Passed
- All 5 files created/modified correctly
- All 6 entry types fully supported with type-specific forms
- Password generator uses SecRandomCopyBytes for cryptographic randomness
- TOTP parsing supports both raw Base32 and otpauth:// URI
- Attachment limits enforced (2MB, 5 files)
- CRUD operations properly wired to MainView and EntryDetailView

**Code Quality Review (First Pass):** ✅ Approved with recommendations
- Excellent architecture with clean separation of concerns
- Security-conscious implementation (crypto RNG, attachment limits)
- Good UX patterns (live previews, validation)
- Two index stability issues found: custom fields and attachments using unstable ForEach identities

**Code Quality Review (After Fixes):** ✅ Approved
- Both index stability issues fixed with enumerated pattern
- Custom fields now use explicit Binding to avoid index capture
- Attachments support duplicate filenames without ID conflicts
- Bounds checks prevent crashes during removal
...
**Task 15 Implementation Results:**

**Commits:**
- cf9fffb: feat(app): global hotkey and floating quick search panel
- 9e83eb3: fix(app): critical hotkey lifecycle bug and state handling

**Spec Compliance Review:** ✅ Passed
- All 5 files created/modified correctly
- Carbon HIToolbox hotkey registration with EventHandlerUPP callback
- CopyTargets maps all 6 entry types per spec §8.3
- FloatingPanel with biometric unlock integration
- Full keyboard navigation (↑↓ Tab ↵ ⌘↵ Esc)
- Live TOTP updates with timer

**Code Quality Review (First Pass):** ⚠️ Critical issues found
1. CRITICAL: EventHandlerUPP lifecycle bug - callback deallocated after register() returns, causing crash on hotkey press
2. Important: Incomplete phase state handling - .firstRun/.corrupted states trigger biometric unlock incorrectly
3. Important: TOTP period protection missing in CopyTargets (inconsistent with QuickPanelView)

**Code Quality Review (After Fixes):** ✅ Approved
- EventHandlerUPP stored as instance property, preventing deallocation crash
- Explicit switch handles all AppState.Phase cases correctly
- TOTP period protection now consistent (max(p.period, 1) in both files)
- Switch pattern is beneficial deviation from plan's guard approach (clearer, more maintainable)
- Build succeeded, 58 tests passed
...
**Task 16 Implementation Results:**

**Commits:**
- 45024c6: feat(core): lock policy (idle threshold decision, pure function)
- 2ba4847: feat(app): lock coordinator wiring idle timer and system events
- 29fa885: fix(app): restart timer on policy change to apply new threshold immediately

**Spec Compliance Review:** ✅ Passed
- All 4 files created/modified correctly
- LockPolicy pure function with 5 tests, all passing
- LockCoordinator with CGEventSource idle monitoring (no accessibility permission needed)
- All 4 system events observed: sleep, lock screen, screen saver, fast user switching
- AppState Combine subscription manages coordinator lifecycle based on phase

**Code Quality Review (First Pass):** ✅ Approved with recommendation
- Excellent architecture: pure function in Core, coordinator in App layer
- Comprehensive test coverage (all edge cases)
- Proper dependency injection for testability
- Memory safety with weak references
- One issue: timer not restarted on policy change while timer already running

**Code Quality Review (After Fix):** ✅ Approved
- Timer lifecycle fixed: always restart on policy update
- Ensures settings changes in Task 17 take effect immediately
...
**Task 17 Implementation Results:**

**Commits:**
- d081659: feat(app): complete settings page with hotkey recorder, security prefs, password change, backup/restore
- 709e2db: docs(app): add educational comment explaining errorMessage naming

**Spec Compliance Review:** ✅ Passed
- All 3 files created/modified correctly
- HotkeyRecorder with NSViewRepresentable key capture, proper cleanup in dismantleNSView
- Four settings sections: Hotkey (recorder + launch at login), Security (auto-lock + clipboard), Master Password (change with validation), Backup/Restore (restore with confirmation)
- Settings persist immediately via unlock.persist()
- Auto-lock changes trigger app.refreshLockPolicy()
- Hotkey changes trigger app.reRegisterHotkey()

**Code Quality Review (First Pass):** ✅ Approved with minor fix
- Excellent architecture with clear separation of concerns
- Proper NSEvent monitor cleanup prevents leaks
- Good integration with lock policy and hotkey registration
- One minor issue: missing educational comment explaining errorMessage naming

**Code Quality Review (After Fix):** ✅ Approved
- Educational comment added explaining Swift catch block error shadowing gotcha
- All plan requirements fulfilled including pedagogical comments
...
**Task 19 Implementation Results:**

**Commit:**
- 0047948: docs: README (positioning, security model, build)

**Implementation:**
- Created README.md with complete documentation
- Positioning: 检索优先密码管理器，解决"遗忘哪些系统有权限"痛点
- Core features: 全局热键浮窗搜索、6种条目类型、Touch ID 一按解锁
- Security model: AES-256-GCM、双层密钥（DEK + 主密码）、自动锁定、剪贴板保护
- Build instructions: xcodegen + Xcode
- Non-goals: 浏览器填充、iOS 同步、导入、密码审计、拼音搜索
- Screenshot placeholders for Task 20
