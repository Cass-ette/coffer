...
struct PasswordGeneratorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text(password.isEmpty ? PasswordGenerator.make(...) : password)
```

上面故意留半截；完整实现如下（实现时整体写入）：
...
struct EntryEditorView: View {
        ...
        Form {
            ...
            Section("自定义字段") {
                ForEach($customFields) { $field in
                    HStack {
                        TextField("名称", text: $field.name)
                        TextField("值", text: $field.value)
                        Toggle("敏感", isOn: $field.isSensitive).toggleStyle(.checkbox)
                        Button(role: .destructive) {
                            customFields.removeAll { $0.name == field.name && $0.value == field.value }
    ...
    private var totpPreview: String? {
        ...
        return TOTPGenerator(config: cfg).code(at: Date())
    }
    ...
    private func buildTOTP() -> TOTPPayload? {
        ...
        if input.lowercased().hasPrefix("otpauth://") {
            guard let url = URL(string: input),
                  ...
                  (try? Base32.decode(secret)) != nil else {
                error = "otpauth 链接无法解析"
                return nil
            }
        ...
        guard (try? Base32.decode(input)) != nil else {
            error = "种子不是合法 Base32"
            return nil
        }
    ...
    private func save() {
        ...
        case .totp:
            guard let p = buildTOTP() else { return }
            payload = .totp(p)
        case .secureNote:
...
enum CopyTargets {
    static func targets(for entry: Entry, at date: Date = Date()) -> [CopyTarget] {
        ...
        case .secureNote(let p):
            return p.body.isEmpty ? [] : [CopyTarget(label: "正文", value: p.body)]
        }
    }
...
final class QuickPanelController: ObservableObject {
    ...
    private func makePanel(app: AppState) -> FloatingPanel {
...
struct QuickPanelView: View {
    ...
    private func currentTOTP(_ entry: Entry) -> String? {
        CopyTargets.targets(for: entry).first?.value
    }
...
1. 启动 app（解锁态），切到别的应用，按 `⌥Space` → 浮窗居中出现、光标在搜索框
2. 敲 2-3 个字 → 结果实时过滤；↑↓ 移动高亮；Tab 看"↵ 复制目标"胶囊循环
3. ↵ → 浮窗关闭；在别的应用里 ⌘V 粘贴出刚才的内容；45 秒后剪贴板被清空
4. totp 条目在结果行内直接显示 6 位实时码；↵ 复制的是码
5. Esc → 浮窗关闭
6. 锁定 app 后按 `⌥Space` → 弹指纹 → 按指纹 → 浮窗直接可用
7. 指纹取消 → 不出现浮窗（回主窗口走主密码）
8. 若本机 Raycast 占用 ⌥Space：浮窗呼不出、设置页（Task 18）显示占用提示——先在 Raycast 改键再验
...
- Create: `assets/brand/icon.svg`（源文件）
...
3. **成功标准 3**：`cp vault.vault /tmp/` 单独检查该文件——`strings /tmp/vault.vault | grep -i <任一条目标题>` 无命中（无明文泄漏）；删掉 /tmp 副本
