...
8. 损坏恢复演练：`echo junk > ~/Library/Application\ Support/Coffer/vault.vault` 后重开 app → 出现恢复页 →"从备份恢复"→ 回锁定页且主密码可解锁
...
struct EntryDetailView: View {
    ...
    private var totpBlock: some View {
        if case .totp(let payload) = entry.payload {
            section("当前验证码") { TOTPCodeView(payload: payload) }
        }
    }
...
手动验证：启动 app → 解锁 → 主界面出现三栏；侧边栏有 4 个预置分组；空列表显示空态。（条目增删改在 Task 14 后才能验。）
